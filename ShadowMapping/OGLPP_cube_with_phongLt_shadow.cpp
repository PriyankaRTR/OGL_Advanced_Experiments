...
GLuint gTexture_Stone, gTexture_Wood, gTexture_Fabric, gTexture_BrickWall;
...
float lightPosition[] = { -3.0f, 4.0f, 0.0f }; //  { -1.0f, 4.5f, -2.0f };  {-2.0f, 4.0f, -1.0f};//{ (r * sin(gAngle)), 0.0f, (r * cos(gAngle) - 5.0) };
...
void initialize(void)
{
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"vec3 s = normalize(vec3(u_light_position - fragPos));" \
	...
	//*(diffuse_light + specular_light)* textureColorValue
	// compile shader
...
void renderScene(Shadow* shadowShader, GLuint typeOfRendering)
{
	...
	vec3 positions[3] = { vec3(0.0f, 2.0f, 0.0f), vec3(2.0f, 0.8f, 1.0f), vec3(-1.0f, 1.0f, 2.0f) };
	//{ vec3(0.0f, 1.0f, -3.5f), vec3(1.0f, 0.5f, -3.0f), vec3(-1.5f, 1.25f, -3.0f) };
	vec3 rotations[3] = { vec3(0.0f,0.0f,0.0f), vec3(0.0f, 40.0f, 0.0f), vec3(60.0f, 0.0f, 60.0f) };
	//{ vec3(0.0f, 45.0f, 0.0f), vec3(20.0f, 0.0f, 10.0f), vec3(0.0f, 30.0f, 10.0f) };
	GLfloat scales[3] = { 0.5f,0.4f,0.25f };


	//modelMatrix = scale(20.0f);
	//modelMatrix = translate(0.0f, -20.0f, 0.0f) * modelMatrix;
	//rotationMatrix = rotate(0.0f, 45.0f, 0.0f); 
...
void normalSceneSetup(void)
{
	...
	if (gbLight == true)
	{
		...
		glUniform3f(gKdUniform, 1.0f, 0.5f, 0.31f);
		glUniform3f(gLaUniform, 1.0f, 1.0f, 1.0f);
		glUniform3f(gKaUniform, 1.0f, 0.5f, 0.31f);
		glUniform3f(gLsUniform, 1.0f, 1.0f, 1.0f);
		glUniform3f(gKsUniform, 1.0f, 0.5f, 0.31f);




		//float lightPosition[] = { -2.0f, 2.0f, -1.0f }; // { (r * sin(gAngle)), 0.0f, (r * cos(gAngle) - 5.0) }; //{ -2.0f, 4.0f, -1.0f };//
...
void renderLightSourceCube(void)
{
	...
	// translate((float)(r * sin(gAngle)), 0.0f, (float)(r * cos(gAngle) - 5.0));//
